...
def _make_translator() -> GeminiTranslator:
    """Build a translator with a fully mocked google.genai client."""
    with patch("src.utils.gemini_client.GEMINI_API_KEY", "test-key"):
        with patch("src.utils.gemini_client.genai.Client") as mock_client_cls:
            mock_client_cls.return_value = MagicMock()
            return GeminiTranslator()


def _set_response(translator: GeminiTranslator, response_text: str) -> MagicMock:
    """Configure the mocked generate_content to return the given text."""
...
class TestTranslateBatch:
    ...
    def test_translate_single_line(self):
        ...
        translator = _make_translator()
        subtitles = [
            SubtitleLine(index=1, start_ms=0, end_ms=2000, text_source="Hello world")
        ]
        _set_response(
            translator,
            json.dumps({"translations": [{"index": 1, "text": "Witaj świecie"}]}),
        )
    ...
    def test_translate_passes_system_instruction_and_model(self):
        """Verify the new SDK call uses system_instruction and configured model."""
        translator = _make_translator()
        subtitles = [
            SubtitleLine(index=1, start_ms=0, end_ms=2000, text_source="Hello")
        ]
        mock_call = _set_response(
            translator,
            json.dumps({"translations": [{"index": 1, "text": "Cześć"}]}),
        )

        translator.translate_batch(subtitles)

        kwargs = mock_call.call_args.kwargs
        assert kwargs["model"] == GeminiTranslator.MODEL_NAME
        assert kwargs["config"].system_instruction == GeminiTranslator.SYSTEM_PROMPT
    ...
    def test_translate_batch_preserves_line_count(self):
        ...
        subtitles = [
            ...
            SubtitleLine(index=2, start_ms=2000, end_ms=4000, text_source="Second"),
            SubtitleLine(index=3, start_ms=4000, end_ms=6000, text_source="Third"),
        ]
        _set_response(
            translator,
            json.dumps(
    ...
    def test_translate_batch_resumes_from_start_batch(self):
        ...
        with patch("src.utils.gemini_client.TRANSLATION_BATCH_SIZE", 2):
            ...
            subtitles = [
                ...
                SubtitleLine(index=2, start_ms=2000, end_ms=4000, text_source="Second"),
                SubtitleLine(index=3, start_ms=4000, end_ms=6000, text_source="Third"),
                SubtitleLine(index=4, start_ms=6000, end_ms=8000, text_source="Fourth"),
            ]
            _set_response(
                translator,
                json.dumps(
    ...
    def test_translate_batch_with_sound_effects(self):
        ...
        subtitles = [
            ...
            SubtitleLine(index=2, start_ms=2000, end_ms=4000, text_source="Hello"),
        ]
        _set_response(
            translator,
            json.dumps(
...
```""",
    ...
    def test_translate_fallback_on_missing_translation(self):
        ...
        subtitles = [
            SubtitleLine(index=1, start_ms=0, end_ms=2000, text_source="Hello"),
            SubtitleLine(index=2, start_ms=2000, end_ms=4000, text_source="World"),
        ]
        _set_response(
            translator,
            json.dumps({"translations": [{"index": 1, "text": "Cześć"}]}),
...
class TestBuildTranslationPrompt:
    ...
    def test_prompt_contains_batch_info(self):
        ...
        translator = _make_translator()
        subtitles = [SubtitleLine(index=1, start_ms=0, end_ms=2000, text_source="Test")]
    ...
    def test_prompt_contains_subtitle_details(self):
        ...
        translator = _make_translator()
        subtitles = [
            SubtitleLine(index=42, start_ms=5000, end_ms=7000, text_source="Hello")
        ]
...
class TestTranslateSingleBatch:
    ...
    def test_invalid_json_response(self):
        ...
        translator = _make_translator()
        subtitles = [SubtitleLine(index=1, start_ms=0, end_ms=2000, text_source="Test")]
        _set_response(translator, "invalid json {")
    ...
    def test_missing_translations_key(self):
        ...
        translator = _make_translator()
        subtitles = [SubtitleLine(index=1, start_ms=0, end_ms=2000, text_source="Test")]
        _set_response(translator, json.dumps({"other_key": "value"}))
...
class TestRateLimitHandling:
    ...
    def test_rate_limit_retry_on_429(self):
        ...
        def mock_generate(*args, **kwargs):
            call_count[0] += 1
            if call_count[0] == 1:
                raise GeminiTranslationError("rate limit exceeded")
            return mock_response

        translator.client.models.generate_content = MagicMock(side_effect=mock_generate)
