"""PipelineService for AI-powered subtitle translation.

This service orchestrates downloading and translating subtitles for video content.
...
class PipelineService(QThread):
    """QThread worker for subtitle translation.
    ...
    def run(self) -> None:
        ...
        It orchestrates the complete flow from subtitle fetch to translation.

        Flow:
            1. Retrieve video file details from DB
            2. Check skip conditions (no subtitle_id, subs already ready)
            3. Fetch subtitles (OpenSubtitles API)
            4. Translate if needed (Gemini API, resumable)
            5. Mark as SUBS_READY
        """
        ...
        try:
            ...
            if video_file.get("pipeline_state") == PipelineState.SUBS_READY.value:
                logger.info(
                    "Subtitles already ready for video_file_id=%d, skipping",
                    self._video_file_id,
                )
                self.signals.pipeline_finished.emit(self._video_file_id, True)
                return
