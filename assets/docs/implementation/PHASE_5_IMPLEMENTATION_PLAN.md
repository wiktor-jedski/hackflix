...
| 21 | Create Gemini Translator Tests | CREATE tests/test_gemini_client.py<br>- Test successful batch translation (verify JSON structure preserved)<br>- Test line count validation (input == output)<br>- Test system/user prompt templates<br>- Test translation of sound effects (should preserve pattern)<br>- Test network error handling with retry logic<br>- Test rate limit error handling<br>- Test batch size enforcement (30 lines per batch)<br>- Test progress tracking via batch index<br>- Mock google.generativeai API calls<br>- Test invalid JSON response handling | assets/docs/architecture/05_VOICEOVER_PIPELINE.md section 3, lines 51-70: Prompt strategy and validation<br>src/config.py line 129: TRANSLATION_BATCH_SIZE = 30<br>Lines 82-86: Testing with mocks for external APIs |
...
| 28 | Update README.md | MODIFY README.md (if exists)<br>- Add FFmpeg to system requirements<br>- Add environment variables: OPENSUBTITLES_API_KEY<br>- Document voiceover generation feature<br>- Add Pi 5 memory optimization notes (10-minute chunks)<br>- Document API dependencies (OpenSubtitles, Gemini, Edge-TTS) | README.md (check if exists, update accordingly) |
| 29 | Update Implementation Workflow Documentation | MODIFY assets/docs/architecture/06_DEV_STANDARDS.md<br>- Update section 6 (Implementation Workflow) to mark Phase 5 as complete<br>- Document Phase 5 completion<br>- Update Phase references in startup requirements | assets/docs/architecture/06_DEV_STANDARDS.md lines 116-129: Implementation workflow with phases 1-5 listed |
---
...
Phase 5d: Testing
18. Create subtitle parser tests
19. Create audio processor tests
20. Create OpenSubtitles client tests
21. Create Gemini translator tests
22. Create Edge-TTS client tests
23. ✅ Create PipelineService Tests
24. Update AppController tests
...
28. Update README.md
29. Update implementation workflow documentation
---
...
- google-generativeai - Translation API
...
- [ ] LibraryView shows pipeline status icons (📝 subtitles, 🎵 generating, 🎙️ ready)
- [ ] All utility modules have comprehensive test coverage with mocking
- [x] PipelineService has comprehensive test coverage (100% of error paths)
- [ ] FFmpeg dependency checking fails fast if missing
...
|------|----------|--------|
| tests/test_pipeline_service.py | 43% | 28 tests covering initialization, signals, skip conditions, error handling, DB persistence. Full integration tests pending utility implementations (audio_processor, opensubtitles_client, gemini_client, edge_tts_client). |
Test Categories
Subtitle Parser Tests (10+ tests):
- Valid SRT parsing
- Various timestamp/text formats
- Sound effect detection patterns
- Malformed SRT handling
- Empty file handling
Audio Processor Tests (12+ tests):
...
OpenSubtitles Client Tests (8+ tests):
- Successful download
- API authentication
- Network errors
- Invalid subtitle ID
- Rate limits
- File I/O errors
Gemini Translator Tests (10+ tests):
...
- Temp file cleanup
- API timeouts
PipelineService Tests (20+ tests):
- Initialization
- Complete pipeline flow
- Skip conditions (null subtitle_id, existing voiceover)
...
- Signal emissions
AppController Pipeline Integration Tests (5+ tests):
