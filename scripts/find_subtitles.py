...
DEFAULT_CATALOG = Path("hackflix-catalog/catalog.json")
...
def main() -> int:
    ...
    for target in targets:
        print(f"\n{target_label(target)}")
        candidate = _find_best_candidate(client, target, args.languages, args.limit)
...
def _filter_targets(targets: list[CatalogTarget], force: bool) -> list[CatalogTarget]:
    if force:
        return targets
    return [target for target in targets if target.subtitle_holder.get("subtitle_id") is None]
...
def _find_best_candidate(
    ...
    for language in languages:
        params = build_search_params(target, language)
