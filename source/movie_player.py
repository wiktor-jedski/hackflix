...
class MoviePlayerApp(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        # ... (Internal State setup remains the same) ...
        ...
        self.cursor_hide_timer = QTimer(self); self.cursor_hide_timer.setInterval(CURSOR_HIDE_TIMEOUT_MS)
        self.cursor_hide_timer.setSingleShot(True); self.cursor_hide_timer.timeout.connect(self.hide_cursor_on_inactivity)
        ...
        self._setup_ui_views_and_layouts()
    ...
    def _setup_ui_views_and_layouts(self):
        ...
        # Player View (Index 0)
        self.player_widget = QWidget(); self.player_layout = QVBoxLayout(self.player_widget)
        self.player_layout.setContentsMargins(0,0,0,0); self.player_layout.setSpacing(0)
        self.video_frame = VideoFrame(); self.control_widget = QWidget()
        self.control_layout = QHBoxLayout(self.control_widget); self.control_layout.setContentsMargins(5,5,5,5)
        ...
        self.back_button = QPushButton(self.tr("Back to Library"))
        ...
        self.tab_widget = QTabWidget(); self.library_tab = FileBrowser()
        self.downloads_tab = DownloadsTab(); self.filmweb_tab = WebBrowserTab()
        self.tab_widget.addTab(self.library_tab, self.tr("Library"))
        self.tab_widget.addTab(self.downloads_tab, self.tr("Downloads"))
        self.tab_widget.addTab(self.filmweb_tab, self.tr("Filmweb"))
        self.browser_layout.addWidget(self.tab_widget)
        # Video Fullscreen View (Index 2)
        self.video_fullscreen_widget = QWidget(); self.video_fullscreen_widget.setObjectName("VideoFullscreenContainer")
    ...
    def _connect_signals(self):
        # ... (Signal connections remain the same as previous version) ...
        # UI Elements
        ...
        self.library_tab.translate_subtitle_requested.connect(self.on_translate_subtitle_requested)
    ...
    def exit_video_fullscreen_layout(self):
        if not self.is_video_layout_fullscreen: return; print("Exiting Video Fullscreen Layout")
        self.show_cursor();
        if self.mouse_pos_before_hide: QCursor.setPos(self.mouse_pos_before_hide); self.mouse_pos_before_hide = None
        print("  Restoring App Normal State."); self.setWindowFlags(self.original_window_flags); self.showNormal()
    ...
    def _set_vlc_window(self, win_id_obj):
        try: win_id = int(win_id_obj); print(f"Setting VLC window ID: {win_id}")
        except (TypeError, ValueError) as e: print(f"Error converting winId: {e}"); QMessageBox.warning(self, self.tr("VLC Error"), self.tr("Invalid window ID.")); return
        try:
        ...
        except Exception as e: print(f"Error setting VLC window: {e}"); err_msg = self.tr("Failed attach video:\n{0}").format(e); QMessageBox.warning(self, self.tr("VLC Error"), err_msg)
    ...
    def _toggle_app_fullscreen_decorated(self):
        ...
        if self.isFullScreen():
            self.showNormal(); print(" Exiting App FS (Decorated)")
            ...
            self.original_geometry_before_fs = self.geometry(); self.showFullScreen(); print(" Entering App FS (Decorated)")
        self.show()
    ...
    # ... (on_find_subtitles_requested, on_subtitle_search_results, on_subtitle_search_error, on_subtitle_selected, on_subtitle_download_ready, _download_subtitle_worker, on_actual_download_finished, on_subtitle_download_error, on_subtitle_login_status, on_subtitle_quota_info unchanged) ...
    @pyqtSlot(str)
    def on_find_subtitles_requested(self, video_path):
        ...
        print(f"Searching subs for '{query}' (Lang: {languages})"); QApplication.setOverrideCursor(Qt.WaitCursor)
